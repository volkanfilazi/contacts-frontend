...
<template>
  <div class="flex flex-col gap-5 justify-center items-center h-screen">
    ...
    <button @click="fetchLogin(email,password)" class="border-[1px] rounded-lg pr-5 pl-5 border-yellow-400 p-2 text-white"><span v-if="!loading">Login</span><span v-if="loading"><div role="status">
    ...
    <p v-if="useAuth.loginErrorMessage" class="border-[1px] border-red-600 p-2 text-red-900">{{ useAuth.loginErrorMessage }}</p>
    <p v-if="useAuth.loginSuccessMessage" class="border-[1px] border-green-600 p-2 text-green-900">{{ useAuth.loginSuccessMessage }}</p>
  </div>
</template>